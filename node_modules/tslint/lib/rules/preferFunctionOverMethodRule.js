...
Rule.metadata = {
    ...
    optionsDescription: (_a = ["\n            \"", "\" excludes public methods.\n            \"", "\" excludes protected methods."], _a.raw = ["\n            \"", "\" excludes public methods.\n            \"", "\" excludes protected methods."], Lint.Utils.dedent(_a, OPTION_ALLOW_PUBLIC, OPTION_ALLOW_PROTECTED)),
    options: {
        type: "string",
...
Rule.FAILURE_STRING = "Method does not use 'this'. Use a function instead.";
exports.Rule = Rule;
var Walker = (function (_super) {
    ...
    Walker.prototype.setThisUsed = function (node) {
        var cur = this.stack[this.stack.length - 1];
        if (cur && !isRecursiveCall(node, cur)) {
            cur.isThisUsed = true;
        }
    };
    Walker.prototype.withThisScope = function (name, recur) {
        this.stack.push({ name: name, isThisUsed: false });
        recur();
        return this.stack.pop().isThisUsed;
    };
    Walker.prototype.shouldWarnForModifiers = function (node) {
...
function methodVisibility(node) {
    return Lint.hasModifier(node.modifiers, ts.SyntaxKind.PrivateKeyword) ? 2 /* Private */ :
        Lint.hasModifier(node.modifiers, ts.SyntaxKind.ProtectedKeyword) ? 1 /* Protected */ :
            0 /* Public */;
}
var _a;
